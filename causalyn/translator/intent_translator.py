...
class IntentSpecification:
    """Structured representation of human intent with C-VPSN coordinates."""
    intent_id: str
    goal: str
...
class IntentTranslator:
    """Translates natural language requirements into Intent Specifications via LLM or formal heuristics."""
    ...
    def translate(self, natural_language: str) -> IntentSpecification:
        ...
        # Attempt LLM extraction if provider is available
        if self.llm_provider and getattr(self.llm_provider, "provider_name", None) != "null" and getattr(self.llm_provider, "__class__", type(None)).__name__ != "NullProvider":
            llm_spec = self._translate_with_llm(natural_language, intent_id)
            if llm_spec:
                return llm_spec
    ...
    def _translate_with_llm(self, natural_language: str, intent_id: str) -> Optional[IntentSpecification]:
        """Translate requirements using frontier LLM with structured JSON schema."""
        try:
            from ..llm.provider import LLMMessage
            system_prompt = (
                "You are the Causalyn C-VPSN Intent Vector Synthesizer.\n"
                "Given a natural language software mutation request, extract a formal Intent Vector as JSON.\n"
                "Output ONLY a raw JSON object with no markdown formatting and these keys:\n"
                "{\n"
                '  "goal": "string",\n'
                '  "target_paths": ["string"],\n'
                '  "auth_scope": "PUBLIC" | "RESTRICTED" | "SYSTEM",\n'
                '  "required_invariants": ["string"],\n'
                '  "forbidden_states": ["string"],\n'
                '  "allowed_operations": ["string"],\n'
                '  "ambient_coordinates": [float, float, float]\n'
                "}"
            )
            messages = [
                LLMMessage(role="system", content=system_prompt),
                LLMMessage(role="user", content=natural_language),
            ]
            response = self.llm_provider.complete(messages, temperature=0.0)
            ...
            if raw.startswith("```"):
                ...
                raw = "\n".join(lines).strip()
            data = json.loads(raw)
            coords = data.get("ambient_coordinates", [0.5, 0.9, 0.95])
            if isinstance(coords, list) and len(coords) == 3:
                coord_tuple = (float(coords[0]), float(coords[1]), float(coords[2]))
            else:
                coord_tuple = (0.5, 0.9, 0.95)

            return IntentSpecification(
                intent_id=intent_id,
                goal=data.get("goal", natural_language[:100]),
                target_paths=data.get("target_paths", []),
                auth_scope=data.get("auth_scope", "PUBLIC"),
                ambient_coordinates=coord_tuple,
